...
// Saved-trip continuation rail using the same image-led visual language and the
// SAME shared card geometry as the Discovery Home — identical width, aspect
// ratio, crop, radius and reserved title area. Opening a card navigates to the
// saved detail page using the immutable snapshot — it never regenerates
// anything. A subtle packing-progress bar is shown over the image when progress
// data exists.
    ...
    >
      <span className={`relative block w-full ${DEST_CARD_ASPECT} overflow-hidden rounded-2xl bg-cinema/40`}>
        ...
        <span className="absolute inset-x-0 bottom-0 p-4">
          ...
          <span className="block text-sm text-on-dark/80 mt-1 line-clamp-1">
